...
class Settings(hurricane.Module):
    ...
    def __init__(self):
        ...
        self.c.register(
            simple_command(
                "lang", self.set_lang_cmd, is_global=True, aliases=["set_lang", "setlang"]
            ),
            simple_command(
                "prefix", self.set_prefix_cmd, is_global=True, aliases=["set_prefix", "setprefix"]
            )
        )
    ...
    async def set_prefix_cmd(self, message: Message, context: CommandContext) -> None:
        ...
        if len(prefix) != 1:
            await message.edit(self.t("invalid_prefix"))
            return

        self.set("prefix", prefix)
        await message.edit(self.t("set_prefix_text"))
